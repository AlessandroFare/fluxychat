...
Console playground before billing, hosted `PUBLIC_DEMO_PUBLISHABLE_KEY` on production, PITR UI, Slack+Telegram OAuth gold path, native pub.dev/CocoaPods, Worker `/v1` pin, status-page SLO. Standard DPA is `/dpa`.
