...
FluxyChat's unified Chat API (P26-A) provides top-level methods that auto-infer the correct adapter from thread IDs and user IDs, so you can perform cross-platform operations without manual adapter lookup.
...
- [Adapter Errors](./adapter-errors.md) — Standardized error hierarchy for adapter operations
- [Chat Singleton](./chat-singleton.md) — Global singleton for thread deserialization
