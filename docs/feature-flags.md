...
Platform, industry, marketplace, and agent-platform routes are **production in the sidebar**. `/labs` is the platform catalog hub (same modules).

`NEXT_PUBLIC_DASHBOARD_LABS` / `NEXT_PUBLIC_DASHBOARD_PREVIEW` default **on**. They are no longer used to hide nav items.
