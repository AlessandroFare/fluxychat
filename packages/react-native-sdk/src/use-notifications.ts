...
export function useNotifications(client: FluxyChatClient | null, options?: { limit?: number; pollMs?: number }) {
  ...
  const reload = React.useCallback(async () => {
    ...
    try { setNotifications(await client.getNotifications(limit)); }
  ...
  const markRead = React.useCallback(async (id: number) => { if (!client) return; await client.markNotificationRead(id); setNotifications((prev) => prev.map((n) => n.id === id ? { ...n, read_at: new Date().toISOString() } : n)); }, [client]);
  const markAllRead = React.useCallback(async () => { if (!client) return; await client.markAllNotificationsRead(); await reload(); }, [client, reload]);
