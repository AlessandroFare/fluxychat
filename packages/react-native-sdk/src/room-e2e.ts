...
async function importAesKey(keyB64: string): Promise<CryptoKey> {
  ...
  if (keyBytes.byteLength !== 32) throw new Error('E2E key must be 32 bytes (base64-encoded)');
  return crypto.subtle.importKey('raw', new Uint8Array(keyBytes), { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
}

export async function encryptE2eContent(plaintext: string, keyB64: string): Promise<string> {
  const key = await importAesKey(keyB64);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode(plaintext));
  return JSON.stringify({ e2e: 1, v: 1, c: bytesToBase64(new Uint8Array(ct)), iv: bytesToBase64(iv) } as FluxyE2eEnvelope);
}

export async function decryptE2eContent(content: string, keyB64: string): Promise<string> {
