...
export interface FluxyDeliverableMessage {
  ...
  attachments?: Array<{ id?: number; kind: string; url: string; name: string; sizeBytes?: number; contentType?: string }>;
  streaming?: boolean;
}
...
export function createClientMessageId(): string {
  if (typeof crypto !== 'undefined' && 'randomUUID' in crypto) return `cmsg_${crypto.randomUUID()}`;
...
export function createOptimisticMessage(input: CreateOptimisticMessageInput): FluxyChatMessageWithDelivery {
  return {
...
export function applyServerMessageAck(
  ...
  const acked: FluxyChatMessageWithDelivery = { ...serverMessage, clientMessageId, deliveryStatus: 'sent', deliveryError: undefined };
...
export function markMessageDeliveryFailed(
  ...
  return messages.map((m) =>
    m.clientMessageId === clientMessageId ? { ...m, deliveryStatus: 'failed' as const, deliveryError: errorMessage } : m,
  );
}
...
export function tryMatchPendingByInbound(
  ...
  if (inbound.userId !== senderUserId && inbound.senderId !== senderUserId) return messages;
  const pendingIdx = messages.findIndex(
    (m) => m.deliveryStatus === 'pending' && m.userId === senderUserId && m.content === inbound.content && (m.parentId ?? null) === (inbound.parentId ?? null),
  );
  if (pendingIdx < 0) return messages;
  const pending = messages[pendingIdx];
  const clientMessageId = pending.clientMessageId;
  const next = [...messages];
  next[pendingIdx] = { ...inbound, clientMessageId, deliveryStatus: 'sent', deliveryError: undefined };
