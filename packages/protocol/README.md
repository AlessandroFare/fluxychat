...
```ts
import { isFluxyInboundEvent, FLUXY_INBOUND_EVENT_TYPES } from "@fluxychat/protocol";
...
1. Add to `@fluxychat/protocol` inbound/outbound lists
2. Update Room DO handler
3. Extend SDK `FluxyChatEvent` union
4. Run `pnpm --filter @fluxychat/protocol test`
