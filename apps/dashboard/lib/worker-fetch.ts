...
function enrichNetworkFetchError(err: unknown, input: RequestInfo | URL): Error {
  ...
  const base =
    err instanceof TypeError
      ? `Cannot reach Worker at ${url}. Check the worker is running (pnpm dev in apps/worker), NEXT_PUBLIC_FLUXYCHAT_WORKER_URL in .env.local, dashboard CSP connect-src, and Worker ALLOWED_ORIGINS (include your dashboard origin, e.g. http://localhost:3000).`
