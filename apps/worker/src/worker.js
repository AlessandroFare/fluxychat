...
import { sanitizeMessageAttachments } from "./lib/message-attachments-sanitize.js";
import {
...
import { sanitizeString } from "./lib/sanitize-string.js";
import {
...
export default {
  fetch: (request, env, ctx) => httpGate.fetch(request, withRuntimeConfig(env), ctx),
  scheduled: (event, env, ctx) => workerScheduled(event, withRuntimeConfig(env), ctx),
  async email(message, env, ctx) {
    const { handleCloudflareEmailMessage } = await import("./lib/email-inbound.js");
    try {
      const result = await handleCloudflareEmailMessage(message, withRuntimeConfig(env));
