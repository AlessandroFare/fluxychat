---
title: "Offline notify: in-app chat (FluxyChat) + SMS/WhatsApp (Sent.dm)"
description: "Use **FluxyChat** when the user is in your product (WebSocket + in-app notifications). Use **[Sent.dm](https://sent.dm)** when they are offline and you need tra"
---
...
Inspired by the Apache-2.0 `sent-dm-typescript` SDK in `/docs (multi-channel fan-out, templates, delivery activity log). **No Sent code is vendored into FluxyChat**; you run this in your own Worker or backend.
...
Register a Sent.dm template `chat_notify` with parameters: `sender`, `preview`, `room_id`, and optionally `media_url`, `media_name`, and `media_type` (`image`, `video`, or `document`) for MMS or WhatsApp media. Define a media template variable in Sent.dm pointing at `media_url`.
...
**Chatsemble-style pattern (GPL, concepts only):** org-scoped rooms + email for invites. FluxyChat equivalent: **in-app notifications** for mentions/DMs + **your** email/SMS bridge via webhooks — see [`chatsemble-concepts.md`](//docs).
...
- **US production:** follow the [US SMS compliance playbook](/docs/operations/us-sms-compliance-playbook) (10DLC, STOP/HELP, opt-out webhook mirror via `sent_dm_contacts`).

## Related

- [Public demo hardening](/docs/cookbook/public-demo-hardening) — Turnstile + origin allowlist for guest demos
- [In-app notifications](../../apps/worker/src/lib/in-app-notifications.js) — mention/DM rows in D1
- [Transport fallback](/docs/cookbook/transport-fallback) — WS → SSE when mobile backgrounded
