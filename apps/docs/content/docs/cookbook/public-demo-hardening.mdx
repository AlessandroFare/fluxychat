---
title: "Public demo & guest token hardening"
description: "The dashboard route `/demo` and Worker `GET|POST /demo/session` mint short-lived guest JWTs for a shared room. Without guards, bots can burn quota and pollute t"
---
...
This checklist covers Turnstile, origin allowlists, rate limits, and guest-token hardening for public demo rooms.
...
| WS connect RL | `RATE_LIMIT_WS_CONNECTIONS_PER_MINUTE` | ✅ optional per-IP |
...
- [Research: free4chat abuse layers](//docs)
