...
FluxyHealth, FluxyFinance, and Cross-Reality Continuity share the **Vertical Studio** pattern: versioned capability events on the room bus, live metrics from `GET /rooms/:id/capabilities/snapshot`, and a **Live workspace** panel for operator actions. Hosted has no SLA. Health is not a HIPAA BAA.
