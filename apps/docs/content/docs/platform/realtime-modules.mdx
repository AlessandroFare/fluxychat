...
<Cards>
  ...
  <Card title="Edu" description="Polls and breakouts on the classroom room." href="/docs/platform/edu" />
  <Card title="Chatbot builder" description="Persisted trigger-action workflows." href="/docs/platform/chatbot-builder" />
...
Industry and live-workspace modules fan out typed frames on the **same room WebSocket** as chat. Subscribe with:
...
Industry workflows use `capability_event` with typed `event.type` (e.g. `edu.session.started`, `event.stage.live`) via `POST /capabilities/events` — see [Vertical platform](/docs/platform/vertical-industries).
