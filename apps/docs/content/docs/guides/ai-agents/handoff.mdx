...
| **Tool approvals** | AI proposes an action; human approves via card button ([tool presets](/docs/ai-tool-presets)) |
...
- [Agent queue](/docs/agent-queue) — SLA, dispositions, wrap-up
- [Custom tools](/docs/custom-tools) — tools the agent can call
- [Thread state](/docs/thread-state) — per-thread KV for agent memory
