...
description: "FluxyChat stream resumption lets users reconnect to in-progress AI responses after a page refresh, network drop, or tab switch without losing partial output."
---
...
- **Room DO:** The Durable Object tracks `activeStreams` and sends `streamState` on WebSocket reconnect
- **DevTools:** The DevTools UI shows active streams and their resume status

## See Also

- [LLM Middleware Guide](/docs/guides/llm-middleware) — Intercept and modify stream behavior
- [Workflow Agent Guide](/docs/guides/workflow-agent) — Durable agent execution that survives restarts
