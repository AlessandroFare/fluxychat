...
**Hosted overlay (no forked Worker):** `GET`/`PUT /admin/projects/:id/publish-config` stores deny-substrings, `guestCanPublish`, and `iotAutoAgentId`. The publish pipeline runs file `onPublish` first, then these D1 rules. Put an agent handle in `iotAutoAgentId` to `invokeAgent` on each `iot.reading` without an `@mention`.
