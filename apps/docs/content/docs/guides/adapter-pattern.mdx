...
description: "Adapter interface and Bridges you wire. Not a finished 14-channel helpdesk."
---

# Multi-Platform Adapter Pattern

The Worker ships an `Adapter<TRawMessage>` interface and format converters. Console **Bridges** are paste-token rows: you create the Slack, Discord, Telegram, WhatsApp, or Teams app and point the webhook at the Worker. This is not a finished 14-channel helpdesk.

## Overview

The adapter pattern abstracts platform-specific concerns behind a common interface so you can add a channel without rewriting the room kernel.

**Interfaces in this repo (not a desk SKU):** Web, Slack, Microsoft Teams, Discord, Telegram, WhatsApp, Google Chat, GitHub, Linear, Matrix, Resend (Email), IRC, Twitch, Line, and a generic API adapter. Gold path today is Bridges for Slack/Discord/Telegram/WhatsApp/Teams that you wire yourself.
...
- [Streaming Markdown Guide](/docs/guides/streaming-markdown) — How markdown renders during streaming
- [Card Builder Guide](/docs/guides/card-builder) — Rich message elements across platforms
