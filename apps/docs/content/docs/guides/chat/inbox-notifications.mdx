...
See [Inbox model](/docs/core/inbox) for item kinds and counter vs unseen.
...
Member JWT is required. `useInbox` opens `GET /ws/inbox` (`connectInbox`). The user channel (`/ws/user/:userId`) is still used for non-inbox user events.
