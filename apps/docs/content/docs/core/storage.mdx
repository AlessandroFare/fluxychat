...
Strokes live on `Y.Array` inside `Y.Map("storage")` (`storage.set("strokes", new Y.Array())`). Cursors stay on `useChat().sendCursor`. Same room DO. The dashboard can render that doc with Excalidraw in the browser. Excalidraw is not a second backend.
