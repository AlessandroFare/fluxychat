...
It is not WebRTC audio. Voice AI latency: [load report](/docs/operations/voice-load-test-report) and `GET /admin/voice-ai/stats` (product SLO P95 ≤ 300ms realtime). Async clips remain `POST /messages/voice`. `/voice-ai` stays labs until production stats are populated.
