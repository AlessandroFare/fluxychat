...
It is not WebRTC audio. Console: [`/voice-ai`](/voice-ai). Media for huddles is LiveKit. Voice AI latency: [load report](/docs/operations/voice-load-test-report) and `GET /admin/voice-ai/stats`. Product SLO P95 ≤ 300ms is a target; treat tables as unit-bench until stats are populated. Async clips remain `POST /messages/voice`.
