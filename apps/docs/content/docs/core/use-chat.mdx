...
Public rooms: `publishableKey` on `FluxyRealtimeProvider`, or `joinPublicRoomAsGuest`. See [Quickstart](/docs/getting-started/quickstart).

## One socket for chat and cursors

Under `FluxyRealtimeProvider`, `useChat` and `useLiveCursors` share `sessionScope` `app`. One JSON WebSocket per room. Yjs is still a second binary connection if you mount `FluxyYjsProvider`.

Two widgets on the same `roomId` (assistant panel + live chat) must pass different `sessionScope` values.
