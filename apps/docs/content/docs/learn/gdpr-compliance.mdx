...
curl -sS -H "Authorization: Bearer $MEMBER_JWT" \
  "$FLUXY_BASE_URL/gdpr/export"
```

Starter hosted plans include export. Erasure:

```bash
curl -sS -X DELETE -H "Authorization: Bearer $OWNER_JWT" \
  "$FLUXY_BASE_URL/gdpr/delete"
```

Also `GET /users/:userId/export` with a member JWT for a JSON dump of that user's messages and memberships.
