...
description: "FluxyChat routes shared Worker AI calls through [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) when gateway env vars are set. Legacy dir"
---
...
Copy `apps/worker/.dev.vars.example` to `.dev.vars` and set gateway vars or `AI_BASE_URL` and `AI_API_KEY`.
