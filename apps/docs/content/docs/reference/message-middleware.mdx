---
title: "Message middleware (inbound pipeline)"
description: "FluxyChat runs every inbound chat message through a shared pipeline before it is stored or broadcast (WebSocket `type: \"message\"` and `POST /messages`)."
---
