...
Platform, industry, marketplace, and agent-platform routes are in the sidebar. `/labs` is the platform catalog hub (same modules).

`NEXT_PUBLIC_DASHBOARD_LABS` / `NEXT_PUBLIC_DASHBOARD_PREVIEW` default **on**. They are not used to hide nav items.
