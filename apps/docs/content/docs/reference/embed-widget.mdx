...
description: "One-line install for a support chat bubble on any website, powered by public guest sessions and optional custom domains."
---
...
Serve `embed.js` from `https://chat.yourcompany.com` so API, iframe, and WebSocket share the same origin. Parent marketing site origins still go in the embed allowlist.
