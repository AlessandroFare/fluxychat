...
| `ThreadDepthExceededError` / `FluxyBlockedError` / `blocked` | 400, `reason: thread_depth_exceeded` | Reply closer to the root; max depth 8 |
...
`GET /rooms/:id/live` returns the room's channel stats plus a `members: [{ userId, userInfo }]` array (aggregated across shards for supergroup rooms). Use it from the SDK to know who's connected right now without waiting for the next `presence` WS event:
...
- It's configurable per-room. Call sites that want the Fluxy-style curve set `baseBackoffMs: 1_000, maxBackoffMs: 8_000` on `connectRoom` options. See [Connection state](/docs/core/connection-state) for fallback behavior.
...
The current defaults favor fast recovery on mobile networks. Use `baseBackoffMs` and `maxBackoffMs` on `connectRoom` when you need the Fluxy profile instead. See [Connection state](/docs/core/connection-state) for fallback behavior.
...
Full operator docs: [docs/dashboard-integration.md](https://github.com/AlessandroFare/fluxychat/blob/main/docs/reference/dashboard-integration).
...
See [docs/cookbook/bot-streaming-fluxy-message-stream.md](https://github.com/AlessandroFare/fluxychat/blob/main/docs/cookbook/bot-streaming-fluxy-message-stream) in the monorepo.
...
See [Production setup](/docs/operations/production-setup) for deploy steps.

## Pusher Channels parity

FluxyChat maps [Pusher Channels](https://pusher.com/docs/channels) features to rooms plus JWT. Full matrix: [Pusher Channels parity](/docs/reference/pusher-channels-parity).
...
See [docs/competitive-parity-p10.md](https://github.com/AlessandroFare/fluxychat/blob/main/docs/competitive-parity-p10).
