...
description: "Client for a FluxyChat Worker (self-hosted or FluxyChat Cloud): rooms, messages, WebSockets, agents, and vertical SDKs."
---
...
See [Connection state](/docs/core/connection-state) for reconnect backoff tuning. The defaults are not changing in this minor; P11-B2 stays in the roadmap as a documented decision rather than a code change.
...
Full operator docs: [Dashboard integration](/docs/reference/dashboard-integration).
...
FluxyChat maps [Pusher Channels](https://pusher.com/docs/channels) features to rooms + JWT. Full matrix: [Pusher Channels parity](/docs/reference/pusher-channels-parity).
...
Adopted from the [Vercel Chat SDK](https://chat-sdk.dev) and [AI SDK](https://sdk.vercel.ai) architecture. See [ROADMAP_EXECUTION.md](/docs/../ROADMAP_EXECUTION) for full details.
