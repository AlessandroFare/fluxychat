...
Your app needs Tailwind (same as `@fluxy-chat/ui`). Member JWT **or** `guest` plus `publishableKey` (`pk_`) on a public room.
...
Pass `token` instead of `guest` when you already have a member JWT.
...
| `token` | `string` | Member JWT (unless `client` or `guest`) |
| `guest` | `boolean` | Call `joinPublicRoomAsGuest` (public room, no JWT) |
| `publishableKey` | `string` | Browser `pk_` key. Required with `guest` on hosted public rooms |
