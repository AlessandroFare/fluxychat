...
`FluxyRealtimeProvider` uses `workerUrl` plus **`authTokenProvider`** (JWT string or async mint) or hosted **`connectUrl`**. It does not take `token` or `config.baseUrl`. Pass `userId` when the JWT has no `sub`. `FluxyYjsProvider` takes **`token`**.
