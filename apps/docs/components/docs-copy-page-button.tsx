...
interface DocsCopyPageButtonProps {
  markdownUrl: string;
  githubUrl?: string;
  className?: string;
}

/** Single split control: copy markdown + dropdown (ChatGPT, Claude, Cursor, …). */
  ...
  return (
    <div
      className={cn(
        ...
        "inline-flex h-8 shrink-0 gap-0 overflow-hidden p-0 [&_svg]:size-3.5",
        className,
      )}
    >
      <button
        ...
        disabled={isLoading}
        onClick={onCopy}
        className="inline-flex items-center gap-2 px-3 hover:bg-fd-accent/80 disabled:opacity-50"
      >
        {checked ? <Check /> : <Copy className="text-fd-muted-foreground" />}
        <span>Copy Page</span>
      </button>
      <div className="w-px self-stretch bg-fd-border" aria-hidden />
      <Popover>
        <PopoverTrigger
          type="button"
          className="inline-flex items-center px-2 hover:bg-fd-accent/80 data-[state=open]:bg-fd-accent data-[state=open]:text-fd-accent-foreground"
        >
